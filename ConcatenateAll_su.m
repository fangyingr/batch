function data_all = ConcatenateAll_su(sbj_name, project_name, block_names, dirs,elecs, freq_band, locktype, plot_params,datatype)
...
for ei = 1:length(elecs)
    ...
    for bi = 1:length(block_names)
        bn = block_names{bi};
        dir_in = [dirs.data_root,'/',datatype,'Data/',freq_band,'/',sbj_name,'/',bn,'/EpochData/'];
...
resultpath=fullfile(dirs.result_root,project_name,sbj_name,'allchannels');
if ~exist(resultpath)
    mkdir(resultpath)
end

save([resultpath,filesep,sbj_name,'_',project_name,'_allchannel_',locktype,'_locked_',freq_band,'.mat'],'data_all')
