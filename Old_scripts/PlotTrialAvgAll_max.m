function PlotTrialAvgAll_max(sbj_name,project_name,block_names,dirs,elecs,freq_band,locktype,column,conds,plot_params,datatype)
...
%     data_all = concatBlocks(sbj_name,block_names,dirs,el,datatype,concatfield,tag);
    ...
    if (plot_params.multielec) % if plotting multiple elecs in same figure (will group all conditions together)
        ...
        h(ei) = PlotTrialAvg_max(data_all,column,conds,plot_params);
        hold on
        ...
        PlotTrialAvg_max(data_all,column,conds,plot_params);
        
        if strcmp(plot_params.label,'name')
            suptitle([strrep(data_all.label,'_','\_'),tagchan])
