...
function PlotAveragePower_memory(sbj_name,project_name,block_names,dirs,elecs1,elecs2,pairing,locktype,freq_band,column,conds,plot_params,regions)
%% INPUTS
...
%.RdYlBu .q0-3{fill:rgb(252,141,89)} .RdYlBu .q1-3{fill:rgb(255,255,191)} .RdYlBu .q2-3{fill:rgb(145,191,219)}
%.Set2 .q0-3{fill:rgb(102,194,165)} .Set2 .q1-3{fill:rgb(252,141,98)} .Set2 .q2-3{fill:rgb(141,160,203)}
% load globalVar
...
plot_params.averagewin=[0.4 0.9];
...
for ei = 1:length(elecnums1)
    ...
    if el1 ~= el2
        ...
        data_tmp1=data_tmp1(newind)%./max(data_tmp1(newind));
        data_tmp2=data_tmp2(newind)%./max(data_tmp2(newind));
        ...
        for ci=1:length(conds)
            h=plot(cond_trial_indx{ci},cond_data_tmp1{ci},'c.','MarkerSize',50,'Color',plot_params.col(1,:))
            hold on
        end
        ...
        %% plot elec2
        eleclabels=[elecnames2{1},'_',elecnames2{ei}];
        ...
        for ci=1:length(conds)
            h=[h,plot(cond_trial_indx{ci},cond_data_tmp2{ci},'c.','MarkerSize',50,'Color',plot_params.col(2,:))]
            hold on
        end
        ...
        %set(gca,'looseInset',[0 0 0 0])
        set(gca,'linewidth',2)
        legend(h,{regions{1},regions{2}},'Location','northeastoutside')
        legend('boxoff')
        box off
        title([regions{1},'-',regions{2},' HFB trial response'],'fontsize',28)
        pn_out = sprintf('%s/%s_%s_%s_%s_%s_%slock_%s.png',dir_out,sbj_name,eleclabels,title_conds,project_name,freq_band,locktype,regions{2});
        savePNG(gcf, 300, pn_out)
        ...
        for ci=1:length(conds)
            plot(cond_data_tmp1{ci},cond_data_tmp2{ci},'c.','MarkerSize',50,'Color',[0.5 0.5 0.5])%plot_params.col(8,:)
            hold on
        end
