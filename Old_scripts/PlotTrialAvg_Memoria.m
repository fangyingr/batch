function h = PlotTrialAvg_Memoria(data,column,conds,plot_params)
...
if plot_params.devidemax
     maxtmp= repmat(max(abs(data.wave),[],2),1,size(data.wave,2));
     data.wave=data.wave./maxtmp;              
end
...
%         plot(data.time,plot_data{ci}', 'Color', [.5 .5 .5]) % plot over non-noisy trials in grey
        ...
        ylim(y_lim)
        xlabel(plot_params.xlabel,'fontsize',36)
        ylabel(plot_params.ylabel,'fontsize',36)
        ...
        if size(data.trialinfo.allonsets,2) > 1
            ...
            for i = 1:length(time_events)
                plot([time_events(i) time_events(i)],ylim,'Color', [0 0 0], 'LineWidth',2)
            end
        else
