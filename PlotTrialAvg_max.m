function h = PlotTrialAvg_max(data,column,conds,plot_params)
...
for ci = 1:ncategs
    plot_data_1{ci} = data.wave(grouped_trials{ci}-1,:);
    plot_data_all_1{ci} = data.wave(grouped_trials_all{ci}-1,:);
end
...
%         plot(data.time,plot_data{ci}', 'Color', [.5 .5 .5]) % plot over non-noisy trials in grey
        ...
        if ~strcmp(plot_params.eb,'none')
            ...
            if strcmp(plot_params.eb,'ste')
               plot_data{ci}=plot_data{ci}./max(abs(plot_data{ci}))
