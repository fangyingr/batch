...
for i=1:length(cate)
    ...
    for ri=1:length(pair_regions)
        ...
        tmp=dataselect.(cate{i}).(rname).(cate{i});
       
        
        sdata.(cate{i}).mean(ri,:)=mean(tmp,1);
        sdata.(cate{i}).se(ri,1:2)=std(tmp,1)/sqrt(size(tmp,1));
        
        [p, observeddifference, effectsize] = permutationTest(tmp(:,1),tmp(:,2),10000);
        sdata.(cate{i}).p(ri)=p;
        sdata.(cate{i}).diff(ri)= observeddifference;
        sdata.(cate{i}).effectsize(ri)=effectsize;
    end
end

[a,b]=fdr(sdata.sot.p,0.05)
