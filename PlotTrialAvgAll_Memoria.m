function PlotTrialAvgAll_Memoria(sbj_name,project_name,block_names,dirs,elecs,freq_band,locktype,column,conds,plot_params,datatype,regions)
...
for ei = 1:length(elecs)
    ...
    if (plot_params.multielec) % if plotting multiple elecs in same figure (will group all conditions together)
        ...
        h(ei) = PlotTrialAvg_Memoria(data_all,column,conds,plot_params);
        hold on
        ymin = min(ymin,min(h(ei).YData));
        ymax = max(ymax,max(h(ei).YData));
        elec_names{ei} = [regions{ei},': ',data_all.label];
...
if plot_params.multielec  % if plotting multiple elecs, create legend based on elec #
    ...
    legend boxoff
    set(leg,'fontsize',plot_params.textsize);
    title_conds = conds{1};
    for ci = 2:length(conds)
        title_conds = [title_conds,'+',conds{ci}];
    end
    title(title_conds,'fontsize', 36)
